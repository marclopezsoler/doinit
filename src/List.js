...
import { query, collection, getDocs, where, addDoc, deleteDoc, doc, updateDoc, writeBatch } from "firebase/firestore";
...
function List() {
  ...
  return (
    <div className="App">
      <div>
        <h2 className="welcome-name">Hi <UserDetails/>!</h2>
      </div>
      ...
      {toDoList.length > 0 ?
        <div className="buttons">
          <button className={`button2 ${activeButton === 1 ? "active" : ""}`} onClick={() => handleButtonClick(1)}>Show all</button>
          <button className={`button2 ${activeButton === 2 ? "active" : ""}`} onClick={() => handleButtonClick(2)}>Pending</button>
          <button className={`button2 ${activeButton === 3 ? "active" : ""}`} onClick={() => handleButtonClick(3)}>Completed</button>
          <button className="button3" onClick={deleteAllElements}>Clear</button>
        </div>
        : <></>
      }
      <div className="list">
        {filteredToDoList.length > 0 ? <></> : <div><p>You currently have no tasks here, start working hard!</p></div>}
        {filteredToDoList.map((item, index) => (
          <div className="element" key={item.id}>
            <div className="check">
            <input type="checkbox" className="checkbox" name="checkbox" id={`checkbox-${item.id}`} value={item.name} checked={checkedItems[item.id] || false} onChange={(event) => handleCheckboxChange(event, item.id)} />
        ...
        {filteredToDoList.length > 0 ? <div className="element2"></div> : <></>}
      </div>
    </div>
  );
}
